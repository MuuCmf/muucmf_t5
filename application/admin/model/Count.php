...
class Count extends Model
{
    ...
    public function dayCount()
    {
        ...
        if(!Db::name('CountLost')->where($map)->find()){
            $this->lostCount();
            $this->remainCount();
            $this->activeCount();
        }
    ...
    private function _doRemainCount($date,$day=1)
    {
        ...
        $data=null;
        $remain=Db::name('CountRemain')->where(array('date'=>$strDayTime))->find();
        if($remain){
            $data=$remain;
        }
    ...
    public function activeCount()
    {
        ...
        if($time===strtotime(time_format($time,'Y-m-01'))){
            ...
            Db::name('CountActive')->add($month_data);
        }
        return true;
    }
    ...
    private function _dayActiveCount($action,$today)
    {
        ...
        $users=Db::name('ActionLog')->where($map)->field('user_id')->select();
